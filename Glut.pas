...
{ Delphi conversion by Tom Nuydens
    (delphi3d@gamedeveloper.org - www.gamedeveloper.org/delphi3d)
  Please remember to bug me with comments on this, instead of Mark Kilgard.
  This unit uses the same system to adapt to different OpenGL drivers as the
  GL and GLu units. Read the comments in GL.pas for details.
  I'm not quite finished here... The teapots work just fine, but I seem to have
  a bit of a problem concerning glutInit. It requires a pointer to the command-line
  string, but when I use GetCommandLine, it produces an access violation...
  Once I find out how to get around this, I'll write a routine to replace glutInit. }
